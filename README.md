...
- 中国地图（重要）：飞控 GPS/MAVLink 是 **WGS-84**。Bing/Google 的中国地图（道路一定、其中国端卫星亦然）是 **GCJ-02「火星坐标」偏移**——在偏移底图上点图，得到的是真实 WGS-84 坐标，但与你看到的影像要素差 ~数百米，于是**压的航点/引导点落错位置**（“车不按航点走”的根因）。本项目**不做坐标纠偏**，因此**只保留在中国本就 WGS-84 对齐的底图**：Esri World Imagery（卫星，默认）、OpenStreetMap（街道）、Esri 地名注记（WGS-84 叠加）。这些底图点击即等于 GPS 坐标，无偏移。**已删除全部 Bing/Google 图源。**
